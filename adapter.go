// Package slack implements a slack adapter for the mili bot library.
package slack
...
// BotAdapter implements a mili.Adapter that reads and writes messages to and
// from Slack.
...
// Adapter returns a new slack Adapter as mili.Module.
//
// Apart from the typical mili.ReceiveMessageEvent event, this adapter also emits
// the mili.UserTypingEvent. The ReceiveMessageEvent.Data field is always a
// pointer to the corresponding github.com/nlopes/slack.MessageEvent instance.
func Adapter(token string, opts ...Option) mili.Module {
	return mili.ModuleFunc(func(miliConf *mili.Config) error {
		conf, err := newConf(token, miliConf, opts)
		if err != nil {
			return err
		}

		a, err := NewAdapter(miliConf.Context, conf)
		if err != nil {
			return err
		}

		miliConf.SetAdapter(a)
		return nil
	})
}

func newConf(token string, miliConf *mili.Config, opts []Option) (Config, error) {
	conf := Config{Token: token, Name: miliConf.Name}
	...
	if conf.Logger == nil {
		conf.Logger = miliConf.Logger("slack")
	}
...
// will usually configure the slack adapter as mili.Module (i.e. using the
...
func newAdapter(ctx context.Context, client slackAPI, events chan slack.RTMEvent, conf Config) (*BotAdapter, error) {
	a := &BotAdapter{
		...
		users:         map[string]mili.User{}, // TODO: cache expiration?
...
// RegisterAt implements the mili.Adapter interface by emitting the slack API
// events to the given brain.
func (a *BotAdapter) RegisterAt(brain *mili.Brain) {
	go a.handleSlackEvents(brain)
}

func (a *BotAdapter) handleSlackEvents(brain *mili.Brain) {
	for msg := range a.events {
		...
		case *slack.UserTypingEvent:
			brain.Emit(mili.UserTypingEvent{
...
func (a *BotAdapter) handleMessageEvent(ev *slack.MessageEvent, brain mili.EventEmitter) {
	...
	brain.Emit(mili.ReceiveMessageEvent{
		Text:     text,
		Channel:  ev.Channel,
...
func (a *BotAdapter) handleReactionAddedEvent(ev *slack.ReactionAddedEvent, brain mili.EventEmitter) {
	if ev.User == a.userID {
	...
	if ev.Item.Type != "message" {
		// reactions for other things except messages is not supported by mili
		return
	}
...
func (a *BotAdapter) userByID(userID string) mili.User {
	a.usersMu.RLock()
...
// Send implements mili.Adapter by sending all received text messages to the
...
// React implements mili.ReactionAwareAdapter by letting the bot attach the given
// reaction to the message.
func (a *BotAdapter) React(reaction reactions.Reaction, msg mili.Message) error {
