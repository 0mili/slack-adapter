...
import (
	...
	"github.com/0mili/mili/militest"
...
var _ mili.Adapter = new(BotAdapter)
...
func TestAdapter_DirectMessages(t *testing.T) {
	...
	expectedEvt := mili.ReceiveMessageEvent{Text: "Hello world", Channel: "D023BB3L2", ID: "1360782400.498405", Data: evt}
...
func TestAdapter_MentionBot(t *testing.T) {
	...
	events := brain.RecordedEvents()
	require.NotEmpty(t, events)
	expectedEvt := mili.ReceiveMessageEvent{Text: evt.Text, Channel: evt.Channel, ID: evt.Timestamp, AuthorID: evt.User, Data: evt}
	assert.Equal(t, expectedEvt, events[0])
}
...
func TestAdapter_MentionBotPrefix(t *testing.T) {
	...
	events := brain.RecordedEvents()
	require.NotEmpty(t, events)
	expectedEvt := mili.ReceiveMessageEvent{Text: "PING", Data: evt}
	assert.Equal(t, expectedEvt, events[0])
}
...
func TestAdapter_UserTypingEvent(t *testing.T) {
	...
	expectedUser := mili.User{ID: "UG96B2SGJ", Name: "JD", RealName: "John Doe"}
	expectedEvt := mili.UserTypingEvent{User: expectedUser, Channel: "C1H9RESGL"}
	assert.Equal(t, expectedEvt, events[0])
}
...
func TestAdapter_UserTypingEventError(t *testing.T) {
	...
	expectedUser := mili.User{ID: "UG96B2SGJ"}
	expectedEvt := mili.UserTypingEvent{User: expectedUser, Channel: "C1H9RESGL"}
	assert.Equal(t, expectedEvt, events[0])
}
